...
def get_leaddoc():
	...
	'''
	courses = frappe.db.sql('desc `tabLead`');
	return courses

@frappe.whitelist(allow_guest=True)
def get_dailycontact(count):
	# courses = frappe.db.sql('select * from `tabLead` where status="Lead" and contact_by is null limit '+count);
	# courses=frappe.db.get_all("Lead", fields=("name","lead_name","contact_number","address","status","creation","email_id","cur_status","father_name","school_name","place_of_school","father_contact","class","section"), filters={"status": "New(Very Cold)"}, limit_page_length=count)
	courses=frappe.db.get_list("Lead", fields=("name","lead_name","contact_number","address","status","creation","email_id","cur_status","father_name","school_name","place_of_school","father_contact","class","section"), filters={"status": "New(Very Cold)"}, limit_page_length=count)
